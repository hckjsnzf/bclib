...
int action_this_db(pst_db pdb, char *psqlstr)
{
	...
	sprintf(sqlstr, "%s %s (", sqlstr, pdb->dbname);
	for(i=0; i<ncol; i++) {
		sprintf(sqlstr, "%s %s", sqlstr, (pdb->dbcol+i)->name);
		if(i != (ncol-1) ) {
			strcat(sqlstr, ",");
		}
	}
	...
	sprintf(sqlstr, "%s values (", sqlstr);
	for(i=0; i<ncol; i++) {
		ZERO(svalue);
...
int read_data_toff(char *psline, char *chr, pst_line pfline)
{
	...
	for(i = 0; i<pfline->num; i++) {
		...
		if( (pfline->linecol+i)->length == 0) { /*字段变长 */
			ncp=0;
			while(n<inlen) {
				if(psline[n] == chr[0])
					break;
			...
			n += (pfline->linecol+i)->length+1; /* add yzw 130712 */
		}
		if(n > inlen+1) {
	...
	int ret =0;
	for(i = 0; i<pfline->num; i++) {
		if ( (pfline->linecol+i)->check_num > 0) {
			ret = 1;
			for(int j = 0; j<(pfline->linecol+i)->check_num; j++) {
				if( strcmp( (pfline->linecol+i)->value, (pfline->linecol+i)->check_value[j] ) == 0) {
					ret = 0;
					break;
				}
			}
