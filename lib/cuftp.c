...
int RequestDataConnect()
{
	...
	if(getsockname(mSocketData,(struct sockaddr *)&tmpaddr,(socklen_t *)&length)<0)
	{
		mErrorCode=1012;
	...
	if(getsockname(mSocketControl,(struct sockaddr*)&tmpaddr,(socklen_t *)&length)<0)
	{
		mErrorCode=1013;
...
int AcceptDataConnect()
{
	...
	if((tmpsock=accept(mSocketData,(struct sockaddr*)&tmpaddr,(socklen_t *)&length))<0)
	{
		mErrorCode=1020;
...
0    失败
 */
int DownloadFile_pasv(char* remotefile,char* localfile)
{
	char  commandbuf[1024];
	FILE  *fp;
	char  recvbuf[2049];
